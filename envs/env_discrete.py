...
class DiscreteActionEnv(object):
    ...
    def __init__(self, map_set, map_num):
        self.env = EnvCore(map_set, map_num)
        self.num_agent = self.env.agent_num
        self.n_agents = self.num_agent
        self.signal_obs_dim = self.env.obs_dim
        self.signal_action_dim = self.env.action_dim
        self.share_observation_dim = self.env.share_obs_dim
        ...
        # 使用全局地图的share observation space
        self.share_observation_space = [spaces.Box(low=-np.inf, high=+np.inf, shape=self.env.share_obs_dim,
                                                   dtype=np.float32) for _ in range(self.num_agent)]
    ...
    def step(self, actions):
        ...
        obs, rews, dones, infos, joint_maps, rescue_masks, available_actions = results
