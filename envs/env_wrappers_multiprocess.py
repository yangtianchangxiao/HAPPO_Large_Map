...
class MultiDummyVecEnv():
    ...
    def raise_difficulty(self):
        self.raise_difficulty_async()
    
    def raise_difficulty_async(self):
    ...
    def step_wait(self):
        ...
        obs, rews, dones, infos, joint_maps, rescue_masks, available_actions = map(np.array, zip(*results))
        ...
        return obs, rews, dones, infos, joint_maps, rescue_masks, available_actions
    ...
    def worker(self, remote, parent_remote, env):
        ...
        while True:
            cmd, data = remote.recv()
            if cmd == 'step':
                obs, reward, done, info, joint_map, rescue_masks, available_actions = env.step(data)
                if done[0]:
                    obs, joint_map = env.reset()
                remote.send((obs, reward, done, info, joint_map, rescue_masks, available_actions))
            elif cmd == 'reset':
                obs, joint_map = env.reset()
                remote.send((obs, joint_map))
            elif cmd == 'raise_difficulty':
                env.raise_difficulty()
            elif cmd == 'close':
                remote.close()
                break
            else:
