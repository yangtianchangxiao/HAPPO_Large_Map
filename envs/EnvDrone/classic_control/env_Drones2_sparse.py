...
class Drones(object):
    def __init__(self, pos, view_range, id, map_size):
        ...
        self.map_processed =  np.zeros((1,60,60), dtype=np.float16)
...
class SearchGrid(gym.Env):
    def __init__(self, map_set, map_num):
        ...
        # self.view_range = 10
        self.view_range = 5
        self.run_time = 5 # 不加载模型时
        # self.run_time = 105 # 加载模型时
    ...
    def step(self, action):
        ...
        # self.get_full_obs()
        self.get_joint_obs(self.MC_iter, action)
        ...
        self.joint_map_process =  np.zeros((60,60))
        
        # 整合 open grid 到 joint_map 里
        # open grid 最终的值在 0~0.3 之间
        self.joint_map_process[self.joint_map[1] > 0] = self.joint_map[1][self.joint_map[1] > 0]
        # 整合 occupied grid 到 joint_map 里
        self.joint_map_process[self.joint_map[2] > 0] = -1
    ...
    def reset(self, *, seed: Optional[int] = None, return_info: bool = False,
        ...
        if self.MC_iter > 1:
            print("error")
        self.get_joint_obs(self.MC_iter, action=[[-1], [-1]])
    ...
    def get_drone_obs(self, drone, direction):  # 获得无人机的观测，这里的drone是类
        drone.observed_obs = []
        drone.unobserved = []
        ...
        for k in range(self.drone_num):
            if self.drone_list[k].id != drone.id:
                # and (self.drone_list[k].pos[0] - drone.pos[0]) ** 2 \
            ...
            for j in range(obs_size):
                ...
                if np.array_equal(obs[i, j], [1, 1, 1]):
                    ...
                    if direction == 0 and i == 0:
                        drone.whole_map[1, x, y] += 0.1
                    elif direction == 1 and i == obs_size - 1:
                        drone.whole_map[1, x, y] += 0.1
                    elif direction == 2 and j == 0:
                        drone.whole_map[1, x, y] += 0.1
                    elif direction == 3 and j == obs_size - 1:
                        drone.whole_map[1, x, y] += 0.1
                    else:
                        drone.whole_map[1, x, y] += 0.1
    ...
    def get_joint_obs(self, time_stamp, action):
        ...
        # print("开始获得联合观测")
        for k in k_list:
            direction = action[k][0]
            self.drone_list[k].individual_observed_obs = 0
            temp = self.get_drone_obs(self.drone_list[k], direction)
            size = temp.shape[0]
    ...
    def state_action_reward_done(self, rescue_masks):  # 这里返回状态值，奖励值，以及游戏是否结束
        ...
        for k, drone in enumerate(self.drone_list):   
            # 计算上次观测下，观测区域内的空白网格的value 和
            x_left = max(drone.pos[0] - self.view_range + 1, 0)
            x_right = min(drone.pos[0] + self.view_range, 60)
            y_up = max(drone.pos[1] - self.view_range + 1, 0)
            y_down = min(drone.pos[1] + self.view_range, 60)
            subarray = drone.whole_map[1][x_left:x_right, y_up:y_down]
            squared_array = np.square(subarray)

            # 检查平方后的数组是否为空
            if squared_array.size > 0:
                # 计算平方后数组的平均值
                self.obs_open_value[k] = np.mean(squared_array)            
            else:
                print("pause, x_left, x_right, y_up, y_down", x_left, x_right, y_up, y_down)
            # 方案1：
            reward_list[k] = - 20*self.obs_open_value[k]
            # 方案2：
            # reward_list[k] =  25* (self.last_obs_open_value[k] - self.obs_open_value[k])
            self.last_obs_open_value[k] = self.obs_open_value[k]
        # target_factor = 10
        # information_gain = 0.1
        # distance_factor = 0.1
        ####################
        ...
        for drone_count, each_map in enumerate(single_map_set):
            ...
            self.drone_list[drone_count].map_processed[0][each_map[1] != 0] = each_map[1][each_map[1] != 0]
    ...
    def init_param(self):
        ...
        self.map_size = 60
        self.drone_num = 2
        self.obs_open_value = [0 for i_agent in range(self.drone_num)]
        self.last_obs_open_value = [0 for i_agent in range(self.drone_num)]
        self.available_actions = np.ones((self.drone_num, 4))
        # Rescue paragrams
        # 已探索区域的比例
