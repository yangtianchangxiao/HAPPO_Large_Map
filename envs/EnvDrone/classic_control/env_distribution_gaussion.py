...
sys.path.append(r"/home/cx/autodl_one_layer/envs/EnvDrone/classic_control/")
...
class SearchGrid(gym.Env):
        ...
        for i in range(obs_size):
            for j in range(obs_size):
                ...
                if np.array_equal(obs[i, j], [1, 1, 1]):
                    ...
                    # 不再是把新区域的值赋为1，而是赋为概率地图上的值
                    drone.whole_map[1, x, y] = self.choose_map[x, y]
                    drone.find_possibility =  drone.find_possibility + drone.whole_map[1, x, y]
    ...
    def state_action_reward_done(self, rescue_masks):  # 这里返回状态值，奖励值，以及游戏是否结束
        ...
        # 时间惩罚state
        time_step_factor = 0 # 从0.1升到0.3 优化一下轨迹
        # 发现新区域的奖励系数
        ...
        for i, single_map in enumerate(single_map_set):
            ...
            self.average_list_true[i] = self.average_list[i] = self.drone_list[i].find_possibility + information_gain * self.drone_list[i].open_information_gain
    ...
    def is_path_available(self, agent_coord, target_coord, width, height, obstacle_coordinates):
        ...
        return False
    # Executing the provided code to generate and plot the Gaussian map

    # Define the complete function and parameters
    def generate_random_weights(self, length):
        weights = [random.random() for _ in range(length)]
        total_weight = sum(weights)
        normalized_weights = [w / total_weight for w in weights]
        return normalized_weights

    def generate_random_sigmas(self, length, sigma_range):
        min_sigma, max_sigma = sigma_range
        return [(random.uniform(min_sigma, max_sigma), random.uniform(min_sigma, max_sigma)) for _ in range(length)]

    def create_normalized_total_gaussian_map(self, shape, centers, sigmas, weights=None):
        height, width = shape
        if weights is None:
        ...
        for (center_x, center_y), (sigma_x, sigma_y), weight in zip(centers, sigmas, weights):
            gauss = np.exp(-(((x - center_x) ** 2 / (2 * sigma_x ** 2)) + ((y - center_y) ** 2 / (2 * sigma_y ** 2))))
            g_map += gauss * weight

        total_sum = np.sum(g_map)
        normalized_map = g_map / total_sum
    ...
    def init_param(self):
        ...
        # Example usage
        shape = (58, 58)  # Shape of the map
        # Set the minimum and maximum thresholds for the number of Gaussians
        min_gaussians = 2
        max_gaussians = 5
        number_of_gaussians = random.randint(min_gaussians, max_gaussians)  # Number of Gaussian distributions
        sigma_range = (10, 30)  # Range for sigma values

        # Generating random weights and sigmas
        random_weights = self.generate_random_weights(number_of_gaussians)
        random_sigmas = self.generate_random_sigmas( number_of_gaussians, sigma_range)

        # Centers for the Gaussian distributions
        centers = [(random.randint(0, 59), random.randint(0, 59)) for _ in range(number_of_gaussians)]

        # Creating the Gaussian map
        self.choose_map = self.create_normalized_total_gaussian_map(shape, centers, random_sigmas, random_weights)        
        new_map = np.ones((60,60))
        new_map[1:59,1:59] = self.choose_map
        self.choose_map = new_map
        ...
        wall = np.argwhere(self.choose_map == 1)  # 获取障碍物的坐标
        for pos in wall:
            # tree_pos = []
            x, y = pos
