...
class SearchGrid(gym.Env):
    def __init__(self, map_set, map_num):
        ...
        self.action_space = spaces.Discrete(4)
        self.run_time = 5  # 10 25 40 45 55 65 75 100 110 140 200 Run run_time steps per game 当不禁止碰撞的时候，我用的参数是1000
        self.init_param()
    ...
    def step(self, action):
        ...
        if self.exploration_prop <= self.generate_threshold:
            ...
            for i, repetition in enumerate(self.agent_repetition):
                if repetition > self.repetition_threshold \
                    ...
                    # print("初始rescue!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
                    # print("rescue_action_list", self.rescue_action_list[i].actions)
            ...
            if len(self.rescue_action_list[i].actions) > 0:
                ...
                self.drone_list[i].path.pop(0)
                indices = np.where(np.array(one_hot) == 1)
                action[i] = indices[0][0]
                rescue_mask[i] = -1 - action[i]
                # print("RRRRRRREEEEESSSSCCCCCCUUUUUEEEEEE")
        ...
        observation, reward, done, info, available_actions = self.state_action_reward_done(rescue_mask)
        ...
        return observation,final_joint_map, reward, done, info, available_actions
    ...
    def reset(self, *, seed: Optional[int] = None, return_info: bool = False,
        ...
        observation, _, _, info, available_actions = self.state_action_reward_done(None)
            ...
            if self.land_mark_map[temp_pos[0], temp_pos[1]] > 0:
                ...
                if self.drone_list[k].whole_map[1, self.drone_list[k].pos[0], self.drone_list[k].pos[1]] > 0:
                    if self.drone_list[k].repetition_count > self.repetition_threshold_for_reward:
                        # try:
                        self.drone_list[k].path, self.drone_list[k].action_list, goal_r, goal_c = generate_path(env=self, id=k, free_zone = self.free_map_rescue, obstacle_map = self.joint_map[2], pos = self.drone_list[k].pos)
                        
                        current_rescue_path_length =  len(self.drone_list[k].action_list)
                        self.drone_list[k].path, self.drone_list[k].action_list, goal_r, goal_c = generate_path(env=self, id=k, free_zone = self.free_map_rescue, obstacle_map = self.joint_map[2], pos = temp_pos)
                        
                        if len(self.drone_list[k].action_list) < current_rescue_path_length:
    ...
    def state_action_reward_done(self, rescue_masks):  # 这里返回状态值，奖励值，以及游戏是否结束
        ...
        for i, drone in enumerate(self.drone_list):
            ...
            # print("agent", i, "drone.rescue_path_reduce_flag is", drone.rescue_path_reduce_flag)
            drone.rescue_path_reduce_flag = None
                # print("该动作有利于脱困")
        ...
        return final_single_map_set, reward_list, done_list, target_found_num, available_action
    ...
    def init_param(self):
        ...
        self.repetition_threshold = 100000 # 基础款MIXER里用的是5
        self.repetition_threshold_for_reward = 1000  if self.run_time < 30 else 3 # 用于指导智能体从死胡同等不好的地方出来
        self.rescue_flag = False
