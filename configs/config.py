...
def get_config():
    ...
    parser.add_argument("--n_rollout_threads", type=int,
                        default=3, help="Number of parallel envs for training rollouts")
    ...
    parser.add_argument("--num_env_steps", type=int, 
                        default=7025600000, help='70256000   Number of environment steps to train : happo curriculum learning is 4391*1000*16 = 70,256,000')
    parser.add_argument("--user_name", type=str, 
                        default='happo',help="[for wandb usage], to specify user's name for simply collecting training data.")
    # env parameters
    parser.add_argument("--env_name", type=str, 
                        default='SearchGrid', help="specify the name of environment")
    ...
    parser.add_argument("--batch_expand_times", type=int,
                        default=1, help="Cumulate several times grandients and update network when mini batch number are times of batch_expand_times")

    # network parameters
    parser.add_argument("--nn_type", type=str,
                        default='mlp', choices=["mlp", "cnn", "mixer", 'resnet18', 'attention', 'attention_modified','attention_dense',
                                                'attention_batchnorm', 'attention_batchnorm_dense', 'attention_batchnorm_modified', 'attention_resnet','attention_resnet_120', 'attention_resnet_120_actor_critic'])
    ...
    parser.add_argument("--dropout_prob", type=float, default=0.2,
                        help="Set the dropout_prob which is between 0 and 1. With a larger value, more units will be "
                             "discarded when calculating forward")
    ...
    parser.add_argument("--hidden_size", type=int, 
                        default=512, help="Dimension of hidden layers for actor/critic networks")
    parser.add_argument("--layer_N", type=int, 
                        default=4, help="Number of layers for actor/critic networks")
