...
class Runner(object):
    ...
    def save(self):
        for agent_id in range(self.num_agents):
            if self.use_single_network:
                ...
                model_filename = str(self.save_dir) + "/model_agent" + str(agent_id) + ".pt"
                ...
                policy_actor_optimizer = self.trainer[agent_id].policy.actor_optimizer
                policy_critic_optimizer = self.trainer[agent_id].policy.critic_optimizer

                actor_filename = str(self.save_dir) + "/actor_agent" + str(agent_id) + ".pt"
                critic_filename = str(self.save_dir) + "/critic_agent" + str(agent_id) + ".pt"
                actor_optimizer_filename = str(self.save_dir) + "/actor_optimizer" + str(agent_id) + ".pt"
                critic_optimizer_filename = str(self.save_dir) + "/critic_optimizer" + str(agent_id) + ".pt"

                # 删除原来的文件
                if os.path.exists(actor_filename):
                    os.remove(actor_filename)
                if os.path.exists(critic_filename):
                    os.remove(critic_filename)
                if os.path.exists(actor_optimizer_filename):
                    os.remove(actor_optimizer_filename)
                if os.path.exists(critic_optimizer_filename):
                    os.remove(critic_optimizer_filename)


                # 保存新的模型
                torch.save(policy_actor.state_dict(), actor_filename)
                torch.save(policy_critic.state_dict(), critic_filename)
                torch.save(policy_actor_optimizer.state_dict(), actor_optimizer_filename)
                torch.save(policy_critic_optimizer.state_dict(), critic_optimizer_filename)
    ...
    def save_copy(self):
        for agent_id in range(self.num_agents):
            if self.use_single_network:
                policy_model = self.trainer[agent_id].policy.model
                model_filename = str(self.save_dir) + "/model_agent_copy" + str(agent_id) + ".pt"
                ...
                actor_filename = str(self.save_dir) + "/actor_agent_copy" + str(agent_id) + ".pt"
                critic_filename = str(self.save_dir) + "/critic_agent_copy" + str(agent_id) + ".pt"
                actor_optimizer_filename = str(self.save_dir) + "/actor_optimizer_copy" + str(agent_id) + ".pt"
                critic_optimizer_filename = str(self.save_dir) + "/critic_optimizer_copy" + str(agent_id) + ".pt"
