...
class HAPPO_Policy:
    ...
    def lr_decay(self, episode, episodes):
        ...
        lr_path = self.args.lr_path
        critic_lr_path = self.args.critic_lr_path
        with open(lr_path, "w") as f:
            f.write(str(self.lr))
        with open(critic_lr_path, "w") as f:
            f.write(str(self.critic_lr))
    ...
    def get_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, masks, available_actions=None,
        ...
        """
        actions, action_log_probs, rnn_states_actor, soft_prob = self.actor(obs,
                                                                 rnn_states_actor,
                                                                 masks,
                                                                 available_actions,
                                                                 deterministic)

        # print("cent_obs",cent_obs.shape)
        values, rnn_states_critic = self.critic(cent_obs, rnn_states_critic, masks)
        return values, actions, action_log_probs, rnn_states_actor, rnn_states_critic, soft_prob
    ...
    def get_values(self, cent_obs, rnn_states_critic, masks):
        ...
        :param cent_obs (np.ndarray): centralized input to the critic.
        :param rnn_states_critic: (np.ndarray) if critic is RNN, RNN states for critic.
        :param masks: (np.ndarraly) denotes points at which RNN states should be reset.
    ...
    def evaluate_actions(self, cent_obs, obs, rnn_states_actor, rnn_states_critic, action, masks,
        ...
        action_log_probs, dist_entropy, soft_probs = self.actor.evaluate_actions(obs,
                                                                rnn_states_actor,
                                                                action,
                                                                masks,
                                                                available_actions,
        ...
        values, _ = self.critic(cent_obs, rnn_states_critic, masks)
        return values, action_log_probs, dist_entropy, soft_probs
