...
class resnet(nn.Module):
    def __init__(self, dropout_prob):
        ...
        self.conv_local_view = nn.Conv2d(in_channels=2, out_channels=32, kernel_size=3, stride=2, padding=0) # input 9*9; output （9-2+2*0）/2+1=5
        self.bath_norm_local_view = nn.LayerNorm([32, 9, 9])
    ...
    def forward(self, x, batch_size):
        ...
        x2 = x[:, :, 60*60:].view(batch_size, -1, 19, 19)
...
# Shape of residual_x: torch.Size([32, 4, 62, 62])
        ...
        x2 = self.local_conv(x2) # output batch_size, 32, 9, 9
        return x, x2
...
class Attention_model(nn.Module):
    def __init__(self, args, obs_shape):
        ...
        # )
        mlp_input = int(d_model*(15*15+9*9)) # I have half size of map twice
        self.mlp = nn.Linear(mlp_input, hidden_dim)
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.dropout = nn.Dropout(dropout_prob)
        self.state_length = 60*60+2*19*19 # 19*19 is local_view_range
    ...
    def forward(self, x):
        ...
        x, x2 = self.resnet(x, batch_size) # x shape: [batch_size, 32, 15, 15]; x2 shape: [batch_size, 32, 9, 9]
